    ...
    if (vst3Format != nullptr) {
        ...
        for (const auto &vstDir : vstDirectories) {
            if (vstDir.exists() && vstDir.isDirectory()) {
                ...
                DBG("Directory: " + vstDir.getFullPathName() + " contains " + juce::String(vstBundles.size()) +
                    " .vst3 bundles");
                ...
                for (const auto &vstBundle : vstBundles) {
                    if (addedCount >= 3)
                        break; // Limit to 3 plugins for testing
                    ...
                    if (validBundle) {
                        ...
                        if (foundDescription) {
                            ...
                            info.numInputChannels = description->numInputChannels;
                            info.numOutputChannels = description->numOutputChannels;
                            info.isInstrument = description->isInstrument; // Plugin is an instrument if it has no inputs
